...
class DotDir {
  ...
  loadConfig (name) {
    try {
      const config = (() => {
        const _config = JSON.parse(fs.readFileSync(this.configPath()))
        if (_config[name] != null) {
          return Object.assign({name: name}, _config[name])
        }
        if (name == null && _config['default'] != null) {
          return Object.assign({name: 'default'}, _config['default'])
        }
        return null
      })()
      if (config != null) {
        return Object.assign({dataDir: this.dataDirPath()}, config)
      }
    } catch (e) {}
    return null
  }
